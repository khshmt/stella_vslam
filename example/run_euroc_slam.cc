...
int main(int argc, char* argv[]) {
    ...
    // run tracking
    if (cfg->yaml_node_["Camera"]["setup"].as<std::string>() == stella_vslam::camera::setup_type_to_string[0]) {
        mono_tracking(cfg, vocab_file_path->value(), data_dir_path->value(),
                      frame_skip->value(), no_sleep->is_set(), auto_term->is_set(),
                      eval_log->is_set(), map_db_path->value(), equal_hist->is_set());
    }
    else if (cfg->yaml_node_["Camera"]["setup"].as<std::string>() == stella_vslam::camera::setup_type_to_string[1]) {
        stereo_tracking(cfg, vocab_file_path->value(), data_dir_path->value(),
                        frame_skip->value(), no_sleep->is_set(), auto_term->is_set(),
                        eval_log->is_set(), map_db_path->value(), equal_hist->is_set());
    }
    else {
        throw std::runtime_error("Invalid setup type: " + cfg->yaml_node_["Camera"]["setup"].as<std::string>());
    }
